...
```typescript
function validateSample<T>(schema: z.ZodType<T>, items: unknown[], sampleSize: number = 100): { valid: boolean; sampleErrors?: z.ZodIssue[] } {
...
```typescript
async function* validateStream<T>(schema: z.ZodType<T>, items: AsyncIterable<unknown>): AsyncGenerator<T, void, unknown> {
