...
const lowercaseEmail = z.preprocess((val) => (typeof val === 'string' ? val.toLowerCase().trim() : val), z.string().email())
...
const numericString = z.preprocess((val) => (typeof val === 'string' ? Number(val) : val), z.number())
...
const csvArray = z.preprocess((val) => (typeof val === 'string' ? val.split(',').map((s) => s.trim()) : val), z.array(z.string()))
```
